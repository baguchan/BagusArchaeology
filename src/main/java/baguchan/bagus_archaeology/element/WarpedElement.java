...
public class WarpedElement extends AlchemyElement {
    ...
    public void entityAttack(Mob entity, Entity target, Item item, float power) {
        ...
        if (!entity.level().isClientSide()) {
            if (target instanceof LivingEntity living) {
                living.addEffect(new MobEffectInstance(MobEffects.POISON, (int) (power * 10F)), entity);
            }
        ...
        if (!projectile.level().isClientSide()) {
            if (hitResult instanceof EntityHitResult entityHitResult) {
                ...
                if (entity instanceof LivingEntity living) {
                    living.addEffect(new MobEffectInstance(MobEffects.POISON, (int) (power * 10F)), projectile.getOwner() != null ? projectile.getOwner() : projectile);
                }
    ...
    public void active(Entity entity, Item item, float power) {
        if (power < 0) {
            if (!entity.level().isClientSide()) {
                if (entity instanceof LivingEntity living) {
                    living.addEffect(new MobEffectInstance(MobEffects.POISON, (int) (-power * 10F)));
                }
            }
        }
    }
